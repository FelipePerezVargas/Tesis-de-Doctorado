...
si <- ggplot(hplc, aes(x=co, y=si)) + geom_point(color="red",fill="blue",shape=21,alpha=0.5,size=4,stroke = 2) +
  geom_smooth(method=lm, col="springgreen4") +
  ggtitle("Curva de calibración Siringaldehído ", subtitle="Cromatografía de Líquidos de Alta Resolución HPLC") +
  ...
  #Fin del tema
  annotate("rect", xmin = -0.3, xmax = 3.7, ymin = 50000, ymax = 55000, fill="white", colour="black") +
  annotate("text", x = 1.7, y = 53000, label = equation(fit), parse = TRUE)
si
#dev.off()
