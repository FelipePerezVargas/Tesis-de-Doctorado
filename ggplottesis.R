...
for(i in 3:length(tequilas)){
lines(tequilas[,1], tequilas[,i], type='l', lwd=2.5)  
}
...
sir<-vector()
#pdf(file = "confur.pdf", width = 8, height = 10)
sir <- read.table("/Users/User/Desktop/porfavor/nsir.txt", header=TRUE, sep="", na.strings="NA", dec=".", strip.white=TRUE)
...
for(i in 1:161){
  ...
  title(main=paste("Calibración Furfural a", 244.5+0.5*i,'(nm)'),font.main=2,font.lab=2,cex.main=1.5)
  legend(0,max(cav), "Ajuste de  2-Acetil Furfural",  pt.bg = "white", lty = 1, col = "deeppink4")
